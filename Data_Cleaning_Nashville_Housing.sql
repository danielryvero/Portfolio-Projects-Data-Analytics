...
--notice that if parcelid is the same and owneraddress, we can safely
...
--now let's try to populate the null values, knowing that uniqueid is unique
--first we check it is working properly
SELECT blank.parcelid, blank.propertyaddress, populated.parcelid, 
		populated.propertyaddress, COALESCE (
		blank.propertyaddress, populated.propertyaddress) AS PopulatedPropertyAddress
...
-- we will be using a substring to solve this, position and lenght to spot the comma and follow from it
...
--now we alter tables and add new columns to it, and values obtained to them
...
SELECT DISTINCT (soldasvacant), COUNT(soldasvacant) AS YesOrNOCount
FROM Nashville_Housing
  ...
  SELECT *,
    ROW_NUMBER() OVER (
		PARTITION BY ParcelID,
					 ...
					 ORDER BY
						parcelid) AS row_num
--this part of the code is for checking it was correctly removed
-- FROM Nashville_Housing)
-- SELECT *
-- FROM duplicates
-- WHERE row_num > 1
